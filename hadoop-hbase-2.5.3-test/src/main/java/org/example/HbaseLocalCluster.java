...
public class HbaseLocalCluster implements MiniCluster {
    ...
    public void configure(Configuration hbaseConfiguration) throws Exception {
        ...
        hbaseConfiguration.set(HConstants.REPLICATION_BULKLOAD_ENABLE_KEY, hbaseWalReplicationEnabled.toString());
        hbaseConfiguration.setBoolean(LocalHBaseCluster.ASSIGN_RANDOM_PORTS,
                hbaseConfiguration.getBoolean(LocalHBaseCluster.ASSIGN_RANDOM_PORTS, true));
        hbaseConfiguration.setBoolean(CommonFSUtils.UNSAFE_STREAM_CAPABILITY_ENFORCE, false);
        if (hbaseConfiguration.getInt(ServerManager.WAIT_ON_REGIONSERVERS_MINTOSTART, -1) == -1) {
            hbaseConfiguration.setInt(ServerManager.WAIT_ON_REGIONSERVERS_MINTOSTART, 1);
        }
        if (hbaseConfiguration.getInt(ServerManager.WAIT_ON_REGIONSERVERS_MAXTOSTART, -1) == -1) {
            hbaseConfiguration.setInt(ServerManager.WAIT_ON_REGIONSERVERS_MAXTOSTART, 1);
        }
        hbaseConfiguration.set("fs.defaultFS", "file:///");
//        HConstants.
//        hbaseConfiguration.set(HConstants.REPLICATION_SCOPE, hbaseWalReplicationEnabled.toString());
    ...
    @Override
    public void cleanUp() {
        FileUtils.deleteFile(hbaseRootDir);
    }

package org.hhoao.test.clickhouse.test.holo;
...
public class FlinkToHoloRePartitionExample {
    private static final transient Logger LOG =
            LoggerFactory.getLogger(FlinkToHoloRePartitionExample.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (sqlFilePath != null) {
            ...
            if (statements.size() != 3) {
                throw new IllegalArgumentException(
                        "sql file should contain 3(sourceDDL, sourceDQL, sinkDDL) statements.");
            }
            ...
            if (sourceDDL == null || sourceDql == null || sinkDDL == null) {
                throw new IllegalArgumentException(
                        "params sourceDDL, sourceDQL, sinkDDL should not be null.");
            }
        }
        ...
        Tuple3<String, TableSchema, Configuration> sinkTableInfo =
                createTable(tableEnv, sourceDDL, sinkDDL);
        ...
        if (sinkTableInfo.f1.getFieldCount() != sourceSchema.getFieldCount()) {
            throw new IllegalArgumentException(
                    "source schema field count not equal to sink schema field count");
        }

        DataStream<Row> rePartitionStream =
                repartitonByHoloDistributionKey(sourceStream, sinkTableInfo.f1, sinkTableInfo.f2);
        ...
        tableEnv.executeSql(
                String.format(
                        "insert into %s select * from %s", sinkTableInfo.f0, rePartitionTable));
    }

    public static DataStream<Row> repartitonByHoloDistributionKey(
            DataStream<Row> sourceStream, TableSchema sinkSchema, Configuration configuration)
        ...
        // sinkSchema可能只包含部分holo物理表的字段，或者与holo物理表的字段顺序不一致，这里拿到distribution
        // key字段在flink侧sinkSchema中的index
        ...
        for (int i = 0; i < keyIndexInHolo.length; i++) {
            ...
            if (!columnNameToIndexmap.containsKey(columnName)) {
                throw new IllegalArgumentException(
                        String.format(
                                "repartition sink need write all distribution keys %s.",
                                Arrays.toString(holoSchema.get().getDistributionKeys())));
            }
        ...
        return sourceStream
                ...
                .partitionCustom(
                new HoloShardPartitioner(), new HoloKeySelector(shardCount, keyIndexInRow));
    }
    ...
    private static Tuple3<String, TableSchema, Configuration> createTable(
            TableEnvironment tableEnv, String sourceDDL, String sinkDDL) {
            ...
            if (currentCatalog.isPresent()) {
                ...
                if (tables.size() > 1) {
                    throw new RuntimeException(
                            String.format(
                                    "don't know which is the special sink table: %s.", tables));
                }
    ...
    protected static List<String> loadSql(String sqlFilePath) throws Exception {

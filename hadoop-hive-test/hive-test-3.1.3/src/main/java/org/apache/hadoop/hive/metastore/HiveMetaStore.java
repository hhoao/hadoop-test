...
public class HiveMetaStore extends ThriftHiveMetastore {
    ...
    @VisibleForTesting static boolean TEST_TIMEOUT_ENABLED = false;
    @VisibleForTesting static long TEST_TIMEOUT_VALUE = -1;
    ...
    public static class HMSHandler extends FacebookBase implements IHMSHandler {
        ...
        private static final ThreadLocal<Map<String, com.codahale.metrics.Timer.Context>>
                timerContexts =
        ...
        static final Logger auditLog =
                LoggerFactory.getLogger(HiveMetaStore.class.getName() + ".audit");
        ...
        private void notifyMetaListeners(String key, String oldValue, String newValue)
        ...
        /**
         * Internal function to notify listeners to revert back to old values of keys that were
         * modified during setMetaConf. This would get called from HiveMetaStore#cleanupRawStore
         */
            ...
            try {
                ...
                if (conf == null) {
                    throw new MetaException(
                            "Unexpected: modifiedConf is non-null but conf is null");
                }
        ...
        // This will return null if the metastore is not being accessed from a metastore Thrift
        // server,
        ...
        public HMSHandler(String name, Configuration conf, boolean init) throws MetaException {
            ...
            if (threadPool == null) {
                synchronized (HMSHandler.class) {
                    int numThreads =
                            MetastoreConf.getIntVar(conf, ConfVars.FS_HANDLER_THREADS_COUNT);
                    threadPool =
                            Executors.newFixedThreadPool(
                                    numThreads,
                                    new ThreadFactoryBuilder()
                                            .setDaemon(true)
                                            .setNameFormat("HMSHandler #%d")
                                            .build());
                }
            }
            if (init) {
        ...
        public void init() throws MetaException {
            initListeners =
                    MetaStoreUtils.getMetaStoreListeners(
                            MetaStoreInitListener.class,
                            conf,
            ...
            alterHandler =
                    ReflectionUtils.newInstance(
                            JavaUtils.getClass(alterHandlerName, AlterHandler.class), conf);
            ...
            if (alwaysThreadsInitialized.compareAndSet(false, true)) {
                ...
                for (String taskName : taskNames) {
                    MetastoreTaskThread task =
                            JavaUtils.newInstance(
                                    JavaUtils.getClass(taskName, MetastoreTaskThread.class));
                    task.setConf(conf);
                    long freq = task.runFrequency(TimeUnit.MILLISECONDS);
                    // For backwards compatibility, since some threads used to be hard coded but
                    // only run if
                    // frequency was > 0
                    if (freq > 0) {
                        ThreadPool.getPool()
                                .scheduleAtFixedRate(task, freq, freq, TimeUnit.MILLISECONDS);
                    }
                }
            }
        ...
        @VisibleForTesting
        public static void createDefaultCatalog(RawStore ms, Warehouse wh)
            ...
            try {
                ...
                if (defaultCat != null && defaultCat.getLocationUri().equals("TBD")) {
                    // One time update issue.  When the new 'hive' catalog is created in an upgrade
                    // the
        ...
        private void createDefaultDB_core(RawStore ms)
         ...
         *
         * <p>This is a potential contention when HiveServer2 using embedded metastore and Metastore
         ...
         *
         * <p>This is a potential contention when HiveServer2 using embedded metastore and Metastore
         * Server try to concurrently invoke createDefaultRoles. If one failed, JDOException was
         * caught for one more time try, if failed again, simply ignored by warning, which meant
         * another succeeds.
         *
         * @throws MetaException
         */
        ...
        private void createDefaultRoles_core() throws MetaException {
            ...
            privs.addToPrivileges(
                    new HiveObjectPrivilege(
                            ...
                            ADMIN,
                            PrincipalType.ROLE,
                            new PrivilegeGrantInfo("All", 0, ADMIN, PrincipalType.ROLE, true),
                            "SQL"));
            try {
         ...
         *
         * <p>This is a potential contention when HiveServer2 using embedded metastore and Metastore
         * Server try to concurrently invoke addAdminUsers. If one failed, JDOException was caught
         * for one more time try, if failed again, simply ignored by warning, which meant another
         * succeeds.
         *
         * @throws MetaException
        ...
        private void addAdminUsers_core() throws MetaException {
            ...
            // Since user names need to be valid unix user names, per IEEE Std 1003.1-2001 they
            // cannot
            // contain comma, so we can safely split above string on comma.

            Iterator<String> users =
                    Splitter.on(",").trimResults().omitEmptyStrings().split(userStr).iterator();
            if (!users.hasNext()) {
                LOG.info(
                        "No user is added in admin role, since config value "
                                + userStr
        ...
        private String startFunction(String function, String extraLogInfo) {
            ...
            logInfo(
                    (getThreadLocalIpAddress() == null
                                    ? ""
                                    : "source:" + getThreadLocalIpAddress() + " ")
                            + function
                            + extraLogInfo);
        ...
        private void startTableFunction(String function, String catName, String db, String tbl) {
            startFunction(function, " : tbl=" + getCatalogQualifiedTableName(catName, db, tbl));
        }
        ...
        private void startPartitionFunction(
                String function, String cat, String db, String tbl, List<String> partVals) {
            startFunction(
                    function,
                    " : tbl="
                            + getCatalogQualifiedTableName(cat, db, tbl)
                            + "["
                            + join(partVals, ",")
                            + "]");
        }
        ...
        private void endFunction(
                String function, boolean successful, Exception e, String inputTableName) {
            ...
            try {
                try {
                    ...
                    throw new AlreadyExistsException(
                            "Catalog " + catalog.getName() + " already exists");
                } catch (NoSuchObjectException e) {
                    // expected
                }
                ...
                if (!MetaStoreUtils.validateName(catalog.getName(), null)) {
                    throw new InvalidObjectException(
                            catalog.getName() + " is not a valid catalog name");
                }
            ...
            try {
                    ...
                    transactionalListenersResponses =
                            MetaStoreListenerNotifier.notifyEvent(
                                    ...
                                    new AlterCatalogEvent(
                                            oldCat.getCatalog(), rqst.getNewCat(), true, this));
                }
                ...
                if ((null != oldCat) && (!listeners.isEmpty())) {
                    MetaStoreListenerNotifier.notifyEvent(
                            ...
                            EventType.ALTER_CATALOG,
                            new AlterCatalogEvent(
                                    oldCat.getCatalog(), rqst.getNewCat(), success, this),
                            null,
                            transactionalListenersResponses,
                            ms);
                }
        ...
        private void dropCatalogCore(String catName)
            ...
            try {
                ...
                if (allDbs != null && !allDbs.isEmpty()) {
                    // It might just be the default, in which case we can drop that one if it's
                    // empty
                    if (allDbs.size() == 1 && allDbs.get(0).equals(DEFAULT_DATABASE_NAME)) {
                        ...
                        throw new InvalidOperationException(
                                "There are non-default databases in the catalog "
                                        + catName
                ...
                if (success) {
                    wh.deleteDir(
                            wh.getDnsPath(new Path(cat.getLocationUri())), false, false, false);
                } else {
                    ms.rollbackTransaction();
                }
        ...
        private void create_database_core(RawStore ms, final Database db)
            ...
            try {
                ...
                if (!wh.isDir(dbPath)) {
                    ...
                    if (!wh.mkdirs(dbPath)) {
                        throw new MetaException(
                                "Unable to create database path "
                                        + dbPath
            ...
            try {
                try {
                    if (null != get_database_core(db.getCatalogName(), db.getName())) {
                        throw new AlreadyExistsException(
                                "Database " + db.getName() + " already exists");
                    }
                } catch (NoSuchObjectException e) {
                    // expected
                }
        ...
        private void drop_database_core(
                RawStore ms,
                String catName,
                final String name,
                final boolean deleteData,
                final boolean cascade)
                throws NoSuchObjectException, InvalidOperationException, MetaException, IOException,
            ...
            try {
                ...
                String catPrependedName =
                        MetaStoreUtils.prependCatalogToDbName(catName, name, conf);
                ...
                if (!cascade) {
                    ...
                    if (!allFunctions.isEmpty()) {
                        throw new InvalidOperationException(
                                "Database "
                                        + db.getName()
                                        + " is not empty. One or more functions exist.");
                    }
                }
                Path path = new Path(db.getLocationUri()).getParent();
                if (!wh.isWritable(path)) {
                    throw new MetaException(
                            "Database not dropped since "
                                    + path
                                    + " is not writable by "
                ...
                final int tableBatchSize =
                        MetastoreConf.getIntVar(conf, ConfVars.BATCH_RETRIEVE_MAX);

                // First pass will drop the materialized views
                List<String> materializedViewNames =
                        get_tables_by_type(name, ".*", TableType.MATERIALIZED_VIEW.toString());
                int startIndex = 0;
                ...
                while (startIndex < materializedViewNames.size()) {
                    int endIndex =
                            Math.min(startIndex + tableBatchSize, materializedViewNames.size());

                    List<Table> materializedViews;
                    try {
                        materializedViews =
                                ms.getTableObjectsByName(
                                        catName,
                                        name,
                                        materializedViewNames.subList(startIndex, endIndex));
                    ...
                    if (materializedViews != null && !materializedViews.isEmpty()) {
                        for (Table materializedView : materializedViews) {
                            if (materializedView.getSd().getLocation() != null) {
                                Path materializedViewPath =
                                        wh.getDnsPath(
                                                new Path(materializedView.getSd().getLocation()));
                                if (!wh.isWritable(materializedViewPath.getParent())) {
                                    throw new MetaException(
                                            "Database metadata not deleted since table: "
                                                    + materializedView.getTableName()
                                                    + " has a parent location "
                                                    + materializedViewPath.getParent()
                ...
                while (startIndex < allTables.size()) {
                    ...
                    try {
                        tables =
                                ms.getTableObjectsByName(
                                        catName, name, allTables.subList(startIndex, endIndex));
                    } catch (UnknownDBException e) {
                        throw new MetaException(e.getMessage());
                    }
                    ...
                    if (tables != null && !tables.isEmpty()) {
                        for (Table table : tables) {
                            // If the table is not external and it might not be in a subdirectory of
                            // the database
                            ...
                            // For each partition in each table, drop the partitions and get a list
                            // of
                            ...
                            partitionPaths =
                                    dropPartitionsAndGetLocations(
                                            ...
                                            deleteData && !isExternal(table));

                            // Drop the table but not its data
                            drop_table(
                                    MetaStoreUtils.prependCatalogToDbName(
                                            table.getCatName(), table.getDbName(), conf),
                                    table.getTableName(),
                                    false);
                        }
        ...
        private boolean isSubdirectory(Path parent, Path other) {
            return other.toString()
                    .startsWith(
                            parent.toString().endsWith(Path.SEPARATOR)
                                    ? parent.toString()
        ...
        public void drop_database(
                final String dbName, final boolean deleteData, final boolean cascade)
                throws NoSuchObjectException, InvalidOperationException, MetaException {
        ...
        private void create_table_core(
                final RawStore ms, final Table tbl, final EnvironmentContext envContext)
                throws AlreadyExistsException, MetaException, InvalidObjectException,
        ...
        private void create_table_core(
            ...
            if (!MetaStoreUtils.validateName(tbl.getTableName(), conf)) {
                throw new InvalidObjectException(
                        tbl.getTableName() + " is not a valid object name");
            }
            ...
            try {
                ...
                if (is_table_exists(ms, tbl.getCatName(), tbl.getDbName(), tbl.getTableName())) {
                    throw new AlreadyExistsException(
                            "Table " + getCatalogQualifiedTableName(tbl) + " already exists");
                }
                ...
                if (tblPath != null) {
                    if (!wh.isDir(tblPath)) {
                        if (!wh.mkdirs(tblPath)) {
                            throw new MetaException(
                                    tblPath + " is not a directory or unable to create one");
                        }
                        madeDir = true;
                    }
                }
                    ...
                    if (foreignKeys != null) {
                        ...
                        for (int i = 0; i < foreignKeySize; i++) {
                            if (foreignKeys.get(i).getFk_name() == null) {
                                foreignKeys
                                        .get(i)
                                        .setFk_name(constraintNames.get(primaryKeySize + i));
                            }
                        ...
                        for (int i = 0; i < uniqueConstraintSize; i++) {
                            if (uniqueConstraints.get(i).getUk_name() == null) {
                                uniqueConstraints
                                        ...
                                        .setUk_name(
                                                constraintNames.get(
                                                        primaryKeySize + foreignKeySize + i));
                            }
                ...
                if (!transactionalListeners.isEmpty()) {
                    ...
                    if (primaryKeys != null && !primaryKeys.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                ...
                                new AddPrimaryKeyEvent(primaryKeys, true, this),
                                envContext);
                    }
                    if (foreignKeys != null && !foreignKeys.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                ...
                                new AddForeignKeyEvent(foreignKeys, true, this),
                                envContext);
                    }
                ...
                if (!listeners.isEmpty()) {
                    ...
                    if (uniqueConstraints != null && !uniqueConstraints.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                listeners,
                                EventType.ADD_UNIQUECONSTRAINT,
                                new AddUniqueConstraintEvent(uniqueConstraints, success, this),
                                envContext);
                    }
                    if (notNullConstraints != null && !notNullConstraints.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                listeners,
                                EventType.ADD_NOTNULLCONSTRAINT,
                                new AddNotNullConstraintEvent(notNullConstraints, success, this),
                                envContext);
                    }
                }
            }
        ...
        public void create_table_with_environment_context(
                final Table tbl, final EnvironmentContext envContext)
                throws AlreadyExistsException, MetaException, InvalidObjectException {
            ...
            try {
                ...
                if (transactionalListeners.size() > 0) {
                    DropConstraintEvent dropConstraintEvent =
                            new DropConstraintEvent(
                                    catName, dbName, tableName, constraintName, true, this);
                    for (MetaStoreEventListener transactionalListener : transactionalListeners) {
        ...
        public void add_primary_key(AddPrimaryKeyRequest req)
            ...
            String constraintName =
                    (primaryKeyCols != null && primaryKeyCols.size() > 0)
                ...
                if (transactionalListeners.size() > 0) {
                    if (primaryKeyCols != null && primaryKeyCols.size() > 0) {
                        AddPrimaryKeyEvent addPrimaryKeyEvent =
                                new AddPrimaryKeyEvent(primaryKeyCols, true, this);
        ...
        public void add_foreign_key(AddForeignKeyRequest req)
            ...
            String constraintName =
                    (foreignKeyCols != null && foreignKeyCols.size() > 0)
                ...
                if (transactionalListeners.size() > 0) {
                    if (foreignKeyCols != null && foreignKeyCols.size() > 0) {
                        AddForeignKeyEvent addForeignKeyEvent =
                                new AddForeignKeyEvent(foreignKeyCols, true, this);
                        for (MetaStoreEventListener transactionalListener :
                                transactionalListeners) {
        ...
        public void add_unique_constraint(AddUniqueConstraintRequest req)
            ...
            String constraintName =
                    (uniqueConstraintCols != null && uniqueConstraintCols.size() > 0)
                ...
                if (transactionalListeners.size() > 0) {
                    if (uniqueConstraintCols != null && uniqueConstraintCols.size() > 0) {
                        AddUniqueConstraintEvent addUniqueConstraintEvent =
                                new AddUniqueConstraintEvent(uniqueConstraintCols, true, this);
                        for (MetaStoreEventListener transactionalListener :
                                transactionalListeners) {
        ...
        public void add_not_null_constraint(AddNotNullConstraintRequest req)
            ...
            String constraintName =
                    (notNullConstraintCols != null && notNullConstraintCols.size() > 0)
                ...
                if (transactionalListeners.size() > 0) {
                    if (notNullConstraintCols != null && notNullConstraintCols.size() > 0) {
                        AddNotNullConstraintEvent addNotNullConstraintEvent =
                                new AddNotNullConstraintEvent(notNullConstraintCols, true, this);
                        for (MetaStoreEventListener transactionalListener :
                                transactionalListeners) {
        ...
        public void add_default_constraint(AddDefaultConstraintRequest req)
            ...
            String constraintName =
                    (defaultConstraintCols != null && defaultConstraintCols.size() > 0)
                            ? defaultConstraintCols.get(0).getDc_name()
                            : "null";
            ...
            try {
                ...
                if (transactionalListeners.size() > 0) {
                    if (defaultConstraintCols != null && defaultConstraintCols.size() > 0) {
                        ...
                        // AddDefaultConstraintEvent addDefaultConstraintEvent = new
                        // AddDefaultConstraintEvent(defaultConstraintCols, true, this);
                        // for (MetaStoreEventListener transactionalListener :
                        // transactionalListeners) {
                ...
                if (!success) {
                    ...
                    for (MetaStoreEventListener listener : listeners) {
                        // AddNotNullConstraintEvent addDefaultConstraintEvent = new
                        // AddNotNullConstraintEvent(defaultConstraintCols, true, this);
        ...
        public void add_check_constraint(AddCheckConstraintRequest req)
            ...
            String constraintName =
                    (checkConstraintCols != null && checkConstraintCols.size() > 0)
                            ? checkConstraintCols.get(0).getDc_name()
                            : "null";
            ...
            try {
                ...
                if (transactionalListeners.size() > 0) {
                    if (checkConstraintCols != null && checkConstraintCols.size() > 0) {
                        ...
                        // AddcheckConstraintEvent addcheckConstraintEvent = new
                        // AddcheckConstraintEvent(checkConstraintCols, true, this);
                        // for (MetaStoreEventListener transactionalListener :
                        // transactionalListeners) {
                ...
                if (!success) {
                    ...
                    for (MetaStoreEventListener listener : listeners) {
                        // AddNotNullConstraintEvent addCheckConstraintEvent = new
                        // AddNotNullConstraintEvent(checkConstraintCols, true, this);
        ...
        private boolean drop_table_core(
                final RawStore ms,
         ...
         *
         * @param tablePath
         * @param ifPurge completely purge the table (skipping trash) while removing data from
         *     warehouse
        ...
        /**
         * Give a list of partitions' locations, tries to delete each one and for each that fails
         * logs an error.
         *
         * @param partPaths
         * @param ifPurge completely purge the partition (skipping trash) while removing data from
         *     warehouse
         ...
         * writable.
         *
         * <p>Drops the metadata for each partition.
         *
         * <p>Provides a list of locations of partitions which may not be subdirectories of
         * tablePath.
         *
         * @param ms
         * @param dbName
         ...
         */
        private List<Path> dropPartitionsAndGetLocations(
                RawStore ms,
                String catName,
                String dbName,
                String tableName,
                ...
                throws MetaException, IOException, NoSuchObjectException, InvalidObjectException,
                        InvalidInputException {
            int partitionBatchSize = MetastoreConf.getIntVar(conf, ConfVars.BATCH_RETRIEVE_MAX);
            ...
            while (true) {
                List<Partition> partsToDelete =
                        ms.getPartitions(catName, dbName, tableName, partitionBatchSize);
                    ...
                    if (checkLocation
                        ...
                        if (tableDnsPath == null
                            ...
                            if (!wh.isWritable(partPath.getParent())) {
                                throw new MetaException(
                                        "Table metadata not deleted since the partition "
                                                + Warehouse.makePartName(
                                                        partitionKeys, part.getValues())
                                                + " has parent location "
                                                + partPath.getParent()
                ...
                for (MetaStoreEventListener listener : listeners) {
                    // No drop part listener events fired for public listeners historically, for
                    // drop table case.
                    // Limiting to internal listeners for now, to avoid unexpected calls for public
                    // listeners.
        ...
        private void updateStatsForTruncate(
                Map<String, String> props, EnvironmentContext environmentContext) {
            if (null == props) {
                return;
            }
            ...
            environmentContext.putToProperties(
                    StatsSetupConst.STATS_GENERATED, StatsSetupConst.TASK);
            // then invalidate column stats
            StatsSetupConst.clearColumnStatsState(props);
            return;
        }

        private void alterPartitionForTruncate(
                final RawStore ms,
                final String catName,
                final String dbName,
            ...
            alterHandler.alterPartition(
                    ms, wh, catName, dbName, tableName, null, partition, environmentContext, this);
        }

        private void alterTableStatsForTruncate(
                final RawStore ms,
                final String catName,
                final String dbName,
                final String tableName,
            ...
            if (partNames == null) {
                if (0 != table.getPartitionKeysSize()) {
                    for (Partition partition :
                            ms.getPartitions(catName, dbName, tableName, Integer.MAX_VALUE)) {
                    ...
                    alterHandler.alterTable(
                            ms, wh, catName, dbName, tableName, table, environmentContext, this);
                }
            } else {
                for (Partition partition :
                        ms.getPartitionsByNames(catName, dbName, tableName, partNames)) {
                    alterPartitionForTruncate(ms, catName, dbName, tableName, table, partition);
                }
            }
            return;
        ...
        private List<Path> getLocationsForTruncate(
                final RawStore ms,
                final String catName,
                final String dbName,
                final String tableName,
        ...
        public void truncate_table(
                final String dbName, final String tableName, List<String> partNames)
                throws NoSuchObjectException, MetaException {
            try {
                String[] parsedDbName = parseDbName(dbName, conf);
                Table tbl =
                        get_table_core(parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tableName);
                boolean isAutopurge =
                        (tbl.isSetParameters()
                ...
                for (Path location :
                    ...
                    if (!org.apache.hadoop.hive.metastore.utils.HdfsUtils.isPathEncrypted(
                                    getConf(), fs.getUri(), location)
                            && !FileUtils.pathHasSnapshotSubDir(location, fs)) {
                        HdfsUtils.HadoopFileStatus status =
                                new HdfsUtils.HadoopFileStatus(getConf(), fs, location);
                        FileStatus targetStatus = fs.getFileStatus(location);
                        String targetGroup = targetStatus == null ? null : targetStatus.getGroup();
                        wh.deleteDir(location, true, isAutopurge, db);
                        fs.mkdirs(location);
                        HdfsUtils.setFullFileStatus(
                                getConf(), status, targetGroup, fs, location, false);
                    } else {
                        FileStatus[] statuses =
                                fs.listStatus(location, FileUtils.HIDDEN_FILES_PATH_FILTER);
        ...
        public GetTableResult get_table_req(GetTableRequest req)
            ...
            return new GetTableResult(
                    getTableInternal(
                            catName, req.getDbName(), req.getTblName(), req.getCapabilities()));
        }

        private Table getTableInternal(
                String catName, String dbname, String name, ClientCapabilities capabilities)
                throws MetaException, NoSuchObjectException {
            if (isInTest) {
                assertClientHasCapability(
                        capabilities,
        ...
        public List<TableMeta> get_table_meta(
                String dbnames, String tblNames, List<String> tblTypes)
                throws MetaException, NoSuchObjectException {
            List<TableMeta> t = null;
            String[] parsedDbName = parseDbName(dbnames, conf);
            startTableFunction(
                    "get_table_metas", parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tblNames);
            Exception ex = null;
            try {
                t =
                        getMS().getTableMeta(
         ...
         * @return A list of tables whose names are in the the list "names" and are retrievable from
         *     the database specified by "dbnames." There is no guarantee of the order of the
         *     returned tables. If there are duplicate names, only one instance of the table will be
         *     returned.
         * @throws MetaException
        ...
        @Override
        @Deprecated
        public List<Table> get_table_objects_by_name(
                final String dbName, final List<String> tableNames)
                throws MetaException, InvalidOperationException, UnknownDBException {
            String[] parsedDbName = parseDbName(dbName, conf);
            return getTableObjectsInternal(
                    parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tableNames, null);
        }
        ...
        public GetTablesResult get_table_objects_by_name_req(GetTablesRequest req)
            ...
            return new GetTablesResult(
                    getTableObjectsInternal(
                            catName, req.getDbName(), req.getTblNames(), req.getCapabilities()));
        }
                ...
                while (startIndex < distinctTableNames.size()) {
                    ...
                    tables.addAll(
                            ms.getTableObjectsByName(
                                    catName,
                                    dbName,
                                    distinctTableNames.subList(startIndex, endIndex)));
                    startIndex = endIndex;
                }
                for (Table t : tables) {
                    if (MetaStoreUtils.isInsertOnlyTableParam(t.getParameters())) {
        ...
        public Materialization get_materialization_invalidation_info(
                final CreationMetadata cm, final String validTxnList) throws MetaException {
            return getTxnHandler().getMaterializationInvalidationInfo(cm, validTxnList);
        }

        @Override
        public void update_creation_metadata(
                String catName, final String dbName, final String tableName, CreationMetadata cm)
                throws MetaException {
            getMS().updateCreationMetadata(catName, dbName, tableName, cm);
        }

        private void assertClientHasCapability(
                ClientCapabilities client, ClientCapability value, String what, String call)
                throws MetaException {
            if (!doesClientHaveCapability(client, value)) {
                throw new MetaException(
                        "Your client does not appear to support "
                                + what
                                + ". To skip"
                                + " capability checks, please set "
                                + ConfVars.CAPABILITY_CHECK.toString()
                                ...
                                + " specify custom client capabilities via "
                                + call
                                + " API.");
            }
        }

        private boolean doesClientHaveCapability(
                ClientCapabilities client, ClientCapability value) {
        ...
        public List<String> get_table_names_by_filter(
            ...
            try {
                if (parsedDbName[CAT_NAME] == null
                        || parsedDbName[CAT_NAME].isEmpty()
                        || parsedDbName[DB_NAME] == null
                        || parsedDbName[DB_NAME].isEmpty()) {
                    throw new UnknownDBException("DB name is null or empty");
                }
                if (filter == null) {
        ...
        private Partition append_partition_common(
                RawStore ms,
                String catName,
                String dbName,
                String tableName,
            ...
            try {
                ...
                MetaStoreUtils.validatePartitionNameCharacters(
                        part_vals, partitionValidationPattern);
                ...
                if (!wh.isDir(partLocation)) {
                    if (!wh.mkdirs(partLocation)) {
                        throw new MetaException(
                                partLocation + " is not a directory or unable to create one");
                    }
                    madeDir = true;
                }
                ...
                if (MetastoreConf.getBoolVar(conf, ConfVars.STATS_AUTO_GATHER)
                    ...
                    MetaStoreUtils.updatePartitionStatsFast(
                            part, tbl, wh, madeDir, false, envContext, true);
                }
        ...
        @Override
        public Partition append_partition(
                final String dbName, final String tableName, final List<String> part_vals)
                throws InvalidObjectException, AlreadyExistsException, MetaException {
        ...
        private List<Partition> add_partitions_core(
            ...
            try {
                ...
                if (tbl == null) {
                    throw new InvalidObjectException(
                            "Unable to add partitions because "
                ...
                for (final Partition part : parts) {
                    if (!part.getTableName().equals(tblName) || !part.getDbName().equals(dbName)) {
                        throw new MetaException(
                                "Partition does not belong to target table "
                                        + getCatalogQualifiedTableName(catName, dbName, tblName)
                                        + ": "
                                        + part);
                    }
                                                            ...
                                                            try {
                                                                boolean madeDir =
                                                                        createLocationForAddedPartition(
                                                                                table, part);
                                                                if (addedPartitions.put(
                                                                                new PartValEqWrapper(
                                                                                        part),
                                                                                madeDir)
                                                                        != null) {
                                                                    // Technically, for ifNotExists
                                                                    // case, we could insert one and
                                                                    // discard the other
                                                                    // because the first one now
                                                                    // "exists", but it seems better
                                                                    // to report the problem
                                                                    // upstream as such a command
                                                                    // doesn't make sense.
                                                                    throw new MetaException(
                                                                            "Duplicate partitions in the list: "
                                                                                    + part);
                                                                }
                                                                initializeAddedPartition(
                                                                        table, part, madeDir);
                                                            } catch (MetaException e) {
                ...
                // Setting success to false to make sure that if the listener fails, rollback
                // happens.
                success = false;
                // Notification is generated for newly created partitions only. The subset of
                // partitions
                ...
                if (!success) {
                    ...
                    for (Map.Entry<PartValEqWrapper, Boolean> e : addedPartitions.entrySet()) {
                        if (e.getValue()) {
                            // we just created this directory - it's not a case of pre-creation, so
                            // we nuke.
                            wh.deleteDir(
                                    new Path(e.getKey().partition.getSd().getLocation()), true, db);
                        }
                    }
                    ...
                    partFutures.add(
                            threadPool.submit(
                                    new Callable<Partition>() {
                                        ...
                                        public Partition call() throws Exception {
                                            ugi.doAs(
                                                    new PrivilegedExceptionAction<Partition>() {
                                                        @Override
        ...
        private boolean startAddPartition(RawStore ms, Partition part, boolean ifNotExists)
                throws TException {
            MetaStoreUtils.validatePartitionNameCharacters(
                    part.getValues(), partitionValidationPattern);
            boolean doesExist =
                    ms.doesPartitionExist(
                            part.getCatName(),
                            part.getDbName(),
                            part.getTableName(),
                            part.getValues());
                ...
                if (tbl.getSd().getLocation() != null) {
                    partLocation =
                            new Path(
                                    tbl.getSd().getLocation(),
                                    Warehouse.makePartName(
                                            tbl.getPartitionKeys(), part.getValues()));
                }
            } else {
                if (tbl.getSd().getLocation() == null) {
        ...
        private void initializeAddedPartition(
            ...
            initializeAddedPartition(
                    tbl, new PartitionSpecProxy.SimplePartitionWrapperIterator(part), madeDir);
        }
        ...
        private void initializeAddedPartition(
            ...
            String inheritProps =
                    MetastoreConf.getVar(conf, ConfVars.PART_INHERIT_TBL_PROPS).trim();
        ...
        private Partition add_partition_core(
        ...
        public Partition add_partition_with_environment_context(
                ...
                throws InvalidObjectException, AlreadyExistsException, MetaException {
            startTableFunction(
                    "add_partition", part.getCatName(), part.getDbName(), part.getTableName());
            Partition ret = null;
            Exception ex = null;
            try {
                ...
                endFunction(
                        ...
                        ret != null,
                        ex,
                        part != null ? part.getTableName() : null);
            }
            return ret;
        }
        ...
        public Partition exchange_partition(
                ...
                throws TException {
            exchange_partitions(
                    partitionSpecs, sourceDbName, sourceTableName, destDbName, destTableName);
        ...
        public List<Partition> exchange_partitions(
            ...
            // No need to check catalog for null as parseDbName() will never return null for the
            // catalog.
            if (partitionSpecs == null
                    || parsedSourceDbName[DB_NAME] == null
                    || sourceTableName == null
                    || parsedDestDbName[DB_NAME] == null
                    || destTableName == null) {
                throw new MetaException(
                        "The DB and table name for the source and destination tables,"
            ...
            Table destinationTable =
                    ms.getTable(
                            parsedDestDbName[CAT_NAME], parsedDestDbName[DB_NAME], destTableName);
            if (destinationTable == null) {
                throw new MetaException(
                        "The destination table "
                                + getCatalogQualifiedTableName(
                                        parsedDestDbName[CAT_NAME],
            ...
            List<String> partVals =
                    MetaStoreUtils.getPvals(sourceTable.getPartitionKeys(), partitionSpecs);
            ...
            List<Partition> partitionsToExchange =
                    get_partitions_ps(sourceDbName, sourceTableName, partVals, (short) -1);
            if (partitionsToExchange == null || partitionsToExchange.isEmpty()) {
                throw new MetaException(
                        "No partition is found with the values "
                                + partitionSpecs
                                + " for the table "
                                + sourceTableName);
            }
            boolean sameColumns =
                    MetaStoreUtils.compareFieldColumns(
                            sourceTable.getSd().getCols(), destinationTable.getSd().getCols());
            boolean samePartitions =
                    MetaStoreUtils.compareFieldColumns(
            ...
            if (destPartitionNames != null && !destPartitionNames.isEmpty()) {
                for (Partition partition : partitionsToExchange) {
                    String partToExchangeName =
                            Warehouse.makePartName(
                                    destinationTable.getPartitionKeys(), partition.getValues());
            ...
            try {
                    ...
                    transactionalListenerResponsesForAddPartition =
                            MetaStoreListenerNotifier.notifyEvent(
                                    ...
                                    new AddPartitionEvent(
                                            destinationTable, destPartitions, true, this));
                ...
                if (!listeners.isEmpty()) {
                    AddPartitionEvent addPartitionEvent =
                            new AddPartitionEvent(destinationTable, destPartitions, success, this);
                    MetaStoreListenerNotifier.notifyEvent(
                            listeners,
                            EventType.ADD_PARTITION,
                            addPartitionEvent,
        ...
        private boolean drop_partition_common(
                RawStore ms,
                String catName,
                String db_name,
                String tbl_name,
            ...
            try {
                ...
                if (!ms.dropPartition(catName, db_name, tbl_name, part_vals)) {
                    ...
                    isSourceOfReplication =
                            ReplChangeManager.isSourceOfReplication(
                                    ms.getDatabase(catName, db_name));
                    success = ms.commitTransaction();
                }
            } finally {
                if (!success) {
                    ...
                    if (!isExternalTbl) {
                        if (mustPurge) {
                            LOG.info(
                                    "dropPartition() will purge "
                                            + partPath
                                            + " directly, skipping trash.");
                        } else {
                            LOG.info(
                                    "dropPartition() will move "
                                            + partPath
                                            + " to trash-directory.");
                        }
        ...
        private void deleteParentRecursive(
                Path parent, int depth, boolean mustPurge, boolean needRecycle)
        ...
        public DropPartitionsResult drop_partitions_req(DropPartitionsRequest request)
            ...
            String catName =
                    request.isSetCatName() ? request.getCatName() : getDefaultCatalog(conf);
            ...
            boolean ignoreProtection =
                    request.isSetIgnoreProtection() && request.isIgnoreProtection();
            ...
            boolean isSourceOfReplication =
                    ReplChangeManager.isSourceOfReplication(ms.getDatabase(catName, dbName));

            try {
                // We need Partition-s for firing events and for result; DN needs MPartition-s to
                // drop.
                // Great... Maybe we could bypass fetching MPartitions by issuing direct SQL
                // deletes.
                    ...
                    for (DropPartitionsExpr expr : spec.getExprs()) {
                        ...
                        boolean hasUnknown =
                                ms.getPartitionsByExpr(
                                        ...
                                        result);
                        if (hasUnknown) {
                            // Expr is built by DDLSA, it should only contain part cols and simple
                            // ops
                        ...
                        if (!ignoreProtection && expr.isSetPartArchiveLevel()) {
                            for (Partition part : parts) {
                                if (MetaStoreUtils.isArchived(part)
                                    ...
                                    throw new MetaException(
                                            "Cannot drop a subset of partitions "
                                                    + " in an archive, partition "
                                                    + part);
                                }
                            }
                        }
        ...
        public Partition get_partition(
                final String db_name, final String tbl_name, final List<String> part_vals)
                throws MetaException, NoSuchObjectException {
            String[] parsedDbName = parseDbName(db_name, conf);
            startPartitionFunction(
                    "get_partition",
        ...
        /**
         * Fire a pre-event for read table operation, if there are any pre-event listeners
         * registered
         */
        ...
        public List<Partition> get_partitions(
                final String db_name, final String tbl_name, final short max_parts)
                throws NoSuchObjectException, MetaException {
            String[] parsedDbName = parseDbName(db_name, conf);
            startTableFunction(
                    "get_partitions", parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tbl_name);
            fireReadTablePreEvent(parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tbl_name);
            List<Partition> ret = null;
            Exception ex = null;
            try {
                checkLimitNumberOfPartitionsByFilter(
                        parsedDbName[CAT_NAME],
                        parsedDbName[DB_NAME],
                        tbl_name,
                        NO_FILTER_STRING,
        ...
        private void checkLimitNumberOfPartitionsByFilter(
                String catName, String dbName, String tblName, String filterString, int maxParts)
                throws TException {
        ...
        private void checkLimitNumberOfPartitionsByExpr(
                String catName, String dbName, String tblName, byte[] filterExpr, int maxParts)
                throws TException {
            if (isPartitionLimitEnabled()) {
                checkLimitNumberOfPartitions(
                        tblName,
                        get_num_partitions_by_expr(catName, dbName, tblName, filterExpr),
                        maxParts);
            }
        }
        ...
        private void checkLimitNumberOfPartitions(String tblName, int numPartitions, int maxToFetch)
            ...
            if (isPartitionLimitEnabled()) {
                int partitionLimit =
                        MetastoreConf.getIntVar(conf, ConfVars.LIMIT_PARTITION_REQUEST);
        ...
        @Override
        public List<PartitionSpec> get_partitions_pspec(
                final String db_name, final String tbl_name, final int max_parts)
                throws NoSuchObjectException, MetaException {
            ...
            try {
                ...
                if (is_partition_spec_grouping_enabled(table)) {
                    partitionSpecs =
                            get_partitionspecs_grouped_by_storage_descriptor(table, partitions);
                } else {
                ...
                endFunction(
                        "get_partitions_pspec",
                        partitionSpecs != null && !partitionSpecs.isEmpty(),
                        null,
                        tbl_name);
            }
        }
        ...
        private List<PartitionSpec> get_partitionspecs_grouped_by_storage_descriptor(
                Table table, List<Partition> partitions)
                throws NoSuchObjectException, MetaException {
            ...
            if (partitionsWithinTableDirectory.containsKey(true)) {
                ...
                ImmutableList<Partition> partsWithinTableDir =
                        partitionsWithinTableDirectory.get(true);
                ...
                for (Map.Entry<StorageDescriptorKey, List<PartitionWithoutSD>> entry :
                        sdToPartList.entrySet()) {
            ...
            if (partitionsWithinTableDirectory.containsKey(false)) {
                List<Partition> partitionsOutsideTableDir =
                        partitionsWithinTableDirectory.get(false);
                if (!partitionsOutsideTableDir.isEmpty()) {
                    ...
                    partListSpec.setPartitionList(
                            new PartitionListComposingSpec(partitionsOutsideTableDir));
        ...
        private PartitionSpec getSharedSDPartSpec(
                Table table, StorageDescriptorKey sdKey, List<PartitionWithoutSD> partitions) {
        ...
        private static boolean is_partition_spec_grouping_enabled(Table table) {
            ...
            return parameters.containsKey("hive.hcatalog.partition.spec.grouping.enabled")
                    && parameters
                            .get("hive.hcatalog.partition.spec.grouping.enabled")
                            .equalsIgnoreCase("true");
        }

        @Override
        public List<String> get_partition_names(
                final String db_name, final String tbl_name, final short max_parts)
                throws NoSuchObjectException, MetaException {
            String[] parsedDbName = parseDbName(db_name, conf);
            startTableFunction(
                    "get_partition_names", parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tbl_name);
            fireReadTablePreEvent(parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tbl_name);
            List<String> ret = null;
            Exception ex = null;
            try {
                ret =
                        getMS().listPartitionNames(
                                        parsedDbName[CAT_NAME],
                                        parsedDbName[DB_NAME],
                                        tbl_name,
                                        max_parts);
        ...
        public PartitionValuesResponse get_partition_values(PartitionValuesRequest request)
            ...
            // This is serious black magic, as the following 2 lines do nothing AFAICT but without
            // them
            // the subsequent call to listPartitionValues fails.
            List<FieldSchema> partCols = new ArrayList<FieldSchema>();
            partCols.add(request.getPartitionKeys().get(0));
            return getMS().listPartitionValues(
                            catName,
                            dbName,
                            tblName,
                            request.getPartitionKeys(),
                            request.isApplyDistinct(),
                            request.getFilter(),
                            request.isAscending(),
                            request.getPartitionOrder(),
                            request.getMaxParts());
        }

        @Override
        public void alter_partition(
                final String db_name, final String tbl_name, final Partition new_part)
                throws TException {
        ...
        private void rename_partition(
            ...
            try {
                ...
                if (part_vals != null && !part_vals.isEmpty()) {
                    MetaStoreUtils.validatePartitionNameCharacters(
                            new_part.getValues(), partitionValidationPattern);
                }
        ...
        public void alter_partitions(
                final String db_name, final String tbl_name, final List<Partition> new_parts)
                throws TException {
        ...
        public void alter_partitions_with_environment_context(
            ...
            String[] parsedDbName = parseDbName(db_name, conf);
            startTableFunction(
                    "alter_partitions", parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tbl_name);
                ...
                for (Partition tmpPart : new_parts) {
                    ...
                    firePreEvent(
                            new PreAlterPartitionEvent(
                                    parsedDbName[DB_NAME], tbl_name, null, tmpPart, this));
                }
                oldParts =
                        alterHandler.alterPartitions(
                                getMS(),
                                wh,
                                parsedDbName[CAT_NAME],
                                parsedDbName[DB_NAME],
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                ...
                                new AlterPartitionEvent(
                                        oldTmpPart, tmpPart, table, false, true, this));
                    }
                }
            } catch (InvalidObjectException e) {
                ex = e;
        ...
        public void alter_table_with_cascade(
                final String dbname, final String name, final Table newTable, final boolean cascade)
                throws InvalidOperationException, MetaException {
            ...
            String[] parsedDbName = parseDbName(dbname, conf);
            alter_table_core(
                    parsedDbName[CAT_NAME], parsedDbName[DB_NAME], name, newTable, envContext);
        }
        ...
        private void alter_table_core(
            ...
            try {
                ...
                alterHandler.alterTable(
                        getMS(), wh, catName, dbname, name, newTable, envContext, this);
                success = true;
            } catch (NoSuchObjectException e) {
        ...
        @Override
        public List<String> get_tables_by_type(
                final String dbname, final String pattern, final String tableType)
                throws MetaException {
            startFunction(
                    "get_tables_by_type",
        ...
        public List<String> get_materialized_views_for_rewriting(final String dbname)
            ...
            try {
                ret =
                        getMS().getMaterializedViewsForRewriting(
                                        parsedDbName[CAT_NAME], parsedDbName[DB_NAME]);
            } catch (Exception e) {
                ex = e;
                if (e instanceof MetaException) {
                    throw (MetaException) e;
        ...
        public List<FieldSchema> get_fields_with_environment_context(
            ...
            try {
                try {
                    tbl =
                            get_table_core(
                                    parsedDbName[CAT_NAME], parsedDbName[DB_NAME], base_table_name);
                } catch (NoSuchObjectException e) {
                    throw new UnknownTableException(e.getMessage());
                }
                if (null == tbl.getSd().getSerdeInfo().getSerializationLib()
                        || MetastoreConf.getStringCollection(
                                        conf, ConfVars.SERDES_USING_METASTORE_FOR_SCHEMA)
            ...
            if (storageSchemaReader == null) {
                String className =
                        MetastoreConf.getVar(
                                conf, MetastoreConf.ConfVars.STORAGE_SCHEMA_READER_IMPL);
        ...
        /**
         * Return the schema of the table. This function includes partition columns in addition to
         * the regular columns.
         *
        ...
        public List<FieldSchema> get_schema_with_environment_context(
            ...
            try {
                ...
                List<FieldSchema> fieldSchemas =
                        get_fields_with_environment_context(db, base_table_name, envContext);
        ...
        /**
         * Returns the value of the given configuration variable name. If the configuration variable
         * with the given name doesn't exist, or if there were an exception thrown while retrieving
         * the variable, or if name is null, defaultValue is returned.
         */
        @Override
        ...
        private List<String> getPartValsFromName(
                RawStore ms, String catName, String dbName, String tblName, String partName)
                throws MetaException, InvalidObjectException {
        ...
        private Partition get_partition_by_name_core(
                final RawStore ms,
                final String catName,
                final String db_name,
                final String tbl_name,
            ...
            if (p == null) {
                throw new NoSuchObjectException(
                        getCatalogQualifiedTableName(catName, db_name, tbl_name)
                                + " partition ("
                                + part_name
                                + ") not found");
            }
        ...
        @Override
        public Partition get_partition_by_name(
            ...
            startFunction(
                    ...
                    ": tbl="
                            + getCatalogQualifiedTableName(
                                    parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tbl_name)
                            + " part="
                            + part_name);
            Partition ret = null;
            Exception ex = null;
        ...
        @Override
        public Partition append_partition_by_name(
                final String db_name, final String tbl_name, final String part_name)
                throws TException {
            return append_partition_by_name_with_environment_context(
                    db_name, tbl_name, part_name, null);
        }
        ...
        private boolean drop_partition_by_name_core(
                final RawStore ms,
                final String catName,
                final String db_name,
                final String tbl_name,
            ...
            return drop_partition_common(
                    ms, catName, db_name, tbl_name, partVals, deleteData, envContext);
        }
        ...
        public boolean drop_partition_by_name(
            ...
            return drop_partition_by_name_with_environment_context(
                    db_name, tbl_name, part_name, deleteData, null);
        }
            ...
            try {
                ...
                ret =
                        get_partitions_ps_with_auth(
                                db_name, tbl_name, part_vals, max_parts, null, null);
            } catch (Exception e) {
                ex = e;
                rethrowException(e);
            } finally {
        ...
        public ColumnStatistics get_table_column_statistics(
                String dbName, String tableName, String colName) throws TException {
            String[] parsedDbName = parseDbName(dbName, conf);
        ...
        public TableStatsResult get_table_statistics_req(TableStatsRequest request)
            ...
            try {
                ...
                result =
                        new TableStatsResult(
                                (cs == null || cs.getStatsObj() == null)
                                        ? Lists.newArrayList()
                                        : cs.getStatsObj());
            } finally {
            ...
            try {
                List<ColumnStatistics> list =
                        getMS().getPartitionColumnStatistics(
                                        ...
                                        parsedDbName[DB_NAME],
                                        tableName,
                                        Lists.newArrayList(convertedPartName),
                                        Lists.newArrayList(colName));
                if (list.isEmpty()) {
                    return null;
                }
                if (list.size() != 1) {
                    throw new MetaException(
                            list.size() + " statistics for single column and partition");
                }
                statsObj = list.get(0);
            } finally {
                endFunction(
                        "get_column_statistics_by_partition", statsObj != null, null, tableName);
            }
            return statsObj;
        }
            ...
            try {
                List<String> partVals =
                        getPartValsFromName(
                                ...
                                convertedPartName);
                ret =
                        getMS().deletePartitionColumnStatistics(
                                        parsedDbName[CAT_NAME],
                                        parsedDbName[DB_NAME],
                                        tableName,
                                        convertedPartName,
        ...
        public boolean delete_table_column_statistics(
            ...
            try {
                ret =
                        getMS().deleteTableColumnStatistics(
                                        parsedDbName[CAT_NAME],
                                        parsedDbName[DB_NAME],
                                        tableName,
                                        colName);
        ...
        public List<Partition> get_partitions_by_filter(
            ...
            try {
                checkLimitNumberOfPartitionsByFilter(
                        parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tblName, filter, maxParts);
                ret =
                        getMS().getPartitionsByFilter(
                                        parsedDbName[CAT_NAME],
                                        parsedDbName[DB_NAME],
                                        tblName,
                                        filter,
        ...
        public List<PartitionSpec> get_part_specs_by_filter(
                final String dbName, final String tblName, final String filter, final int maxParts)
                throws TException {
            ...
            try {
                Table table =
                        get_table_core(parsedDbName[CAT_NAME], parsedDbName[DB_NAME], tblName);
                // Don't pass the parsed db name, as get_partitions_by_filter will parse it itself
                List<Partition> partitions =
                        get_partitions_by_filter(dbName, tblName, filter, (short) maxParts);
        ...
        public PartitionsByExprResult get_partitions_by_expr(PartitionsByExprRequest req)
            ...
            Exception ex = null;
            try {
                checkLimitNumberOfPartitionsByExpr(
                        catName, dbName, tblName, req.getExpr(), UNLIMITED_MAX_PARTITIONS);
                List<Partition> partitions = new LinkedList<>();
                boolean hasUnknownPartitions =
                        getMS().getPartitionsByExpr(
                                        catName,
                                        dbName,
                                        tblName,
                                        req.getExpr(),
        ...
        @Override
        public int get_num_partitions_by_filter(
                final String dbName, final String tblName, final String filter) throws TException {
            String[] parsedDbName = parseDbName(dbName, conf);
            startTableFunction(
                    "get_num_partitions_by_filter",
                    parsedDbName[CAT_NAME],
            ...
            try {
                ret =
                        getMS().getNumPartitionsByFilter(
                                        parsedDbName[CAT_NAME],
                                        parsedDbName[DB_NAME],
                                        tblName,
                                        filter);
        ...
        private int get_num_partitions_by_expr(
                final String catName, final String dbName, final String tblName, final byte[] expr)
                throws TException {
            int ret = -1;
            Exception ex = null;
            try {
        ...
        public List<Partition> get_partitions_by_names(
                final String dbName, final String tblName, final List<String> partNames)
                throws TException {
        ...
        public PrincipalPrivilegeSet get_privilege_set(
            ...
            String catName =
                    hiveObject.isSetCatName() ? hiveObject.getCatName() : getDefaultCatalog(conf);
            if (hiveObject.getObjectType() == HiveObjectType.COLUMN) {
                String partName = getPartName(hiveObject);
                return this.get_column_privilege_set(
                        catName,
                ...
                String partName = getPartName(hiveObject);
                return this.get_partition_privilege_set(
                        catName,
                        hiveObject.getDbName(),
                        hiveObject.getObjectName(),
                        partName,
                ...
                return this.get_db_privilege_set(
                        catName, hiveObject.getDbName(), userName, groupNames);
            } else if (hiveObject.getObjectType() == HiveObjectType.TABLE) {
                return this.get_table_privilege_set(
                        catName,
                        hiveObject.getDbName(),
                        hiveObject.getObjectName(),
                        userName,
        ...
        private String getPartName(HiveObjectRef hiveObject) throws MetaException {
            ...
            if (partValue != null && partValue.size() > 0) {
                try {
                    String catName =
                            hiveObject.isSetCatName()
                                    ? hiveObject.getCatName()
                                    : getDefaultCatalog(conf);
                    Table table =
                            get_table_core(
        ...
        private PrincipalPrivilegeSet get_column_privilege_set(
                String catName,
                final String dbName,
                final String tableName,
        ...
        private PrincipalPrivilegeSet get_db_privilege_set(
                String catName,
                final String dbName,
                final String userName,
                final List<String> groupNames)
        ...
        private PrincipalPrivilegeSet get_partition_privilege_set(
            ...
            try {
                ret =
                        getMS().getPartitionPrivilegeSet(
                                        catName, dbName, tableName, partName, userName, groupNames);
            } catch (MetaException e) {
                throw e;
            } catch (Exception e) {
                throw new RuntimeException(e);
        ...
        private PrincipalPrivilegeSet get_table_privilege_set(
                String catName,
                final String dbName,
                final String tableName,
                final String userName,
            ...
            try {
                ret =
                        getMS().getTablePrivilegeSet(
                                        catName, dbName, tableName, userName, groupNames);
            } catch (MetaException e) {
                throw e;
            } catch (Exception e) {
                throw new RuntimeException(e);
        ...
        public boolean grant_role(
            ...
            if (PUBLIC.equals(roleName)) {
                throw new MetaException(
                        "No user can be added to "
                                + PUBLIC
                                + ". Since all users implicitly"
                                + " belong to "
                                + PUBLIC
                                + " role.");
            }
        ...
        public boolean revoke_role(
                final String roleName, final String userName, final PrincipalType principalType)
                throws TException {
            return revoke_role(roleName, userName, principalType, false);
        }

        private boolean revoke_role(
                final String roleName,
                final String userName,
                final PrincipalType principalType,
                boolean grantOption)
        ...
        public GrantRevokePrivilegeResponse grant_revoke_privileges(
                GrantRevokePrivilegeRequest request) throws TException {
                ...
                case REVOKE:
                    {
                        ...
                        boolean result =
                                revoke_privileges(request.getPrivileges(), revokeGrantOption);
                        response.setSuccess(result);
                        break;
                    }
                default:
        ...
        public GrantRevokePrivilegeResponse refresh_privileges(
            ...
            try {
                boolean result =
                        getMS().refreshPrivileges(
                                        objToRefresh, authorizer, grantRequest.getPrivileges());
                response.setSuccess(result);
            } catch (MetaException e) {
                throw e;
            } catch (Exception e) {
        ...
        private PrincipalPrivilegeSet get_user_privilege_set(
        ...
        public List<HiveObjectPrivilege> list_privileges(
            ...
            if (hiveObject.getObjectType() == HiveObjectType.DATABASE) {
                return list_db_privileges(
                        principalName, principalType, catName, hiveObject.getDbName());
            }
            if (hiveObject.getObjectType() == HiveObjectType.TABLE) {
                return list_table_privileges(
                        principalName,
                        principalType,
                        catName,
                        hiveObject.getDbName(),
            ...
            if (hiveObject.getObjectType() == HiveObjectType.PARTITION) {
                return list_partition_privileges(
                        principalName,
                        principalType,
                        catName,
                        hiveObject.getDbName(),
            ...
            if (hiveObject.getObjectType() == HiveObjectType.COLUMN) {
                if (hiveObject.getPartValues() == null || hiveObject.getPartValues().isEmpty()) {
                    return list_table_column_privileges(
                            principalName,
                            principalType,
                            catName,
                            hiveObject.getDbName(),
                ...
                return list_partition_column_privileges(
                        principalName,
                        principalType,
                        catName,
                        hiveObject.getDbName(),
        ...
        private List<HiveObjectPrivilege> getAllPrivileges(
            ...
            privs.addAll(
                    list_partition_privileges(
                            principalName, principalType, catName, null, null, null));
            privs.addAll(
                    list_table_column_privileges(
                            principalName, principalType, catName, null, null, null));
            privs.addAll(
                    list_partition_column_privileges(
                            principalName, principalType, catName, null, null, null, null));
            return privs;
        }
        ...
        private List<HiveObjectPrivilege> list_partition_column_privileges(
            ...
            try {
                if (dbName == null) {
                    return getMS().listPrincipalPartitionColumnGrantsAll(
                ...
                if (principalName == null) {
                    return getMS().listPartitionColumnGrantsAll(
                                    catName, dbName, tableName, partName, columnName);
                }

                return getMS().listPrincipalPartitionColumnGrants(
                                principalName,
                                principalType,
                                catName,
                                dbName,
        ...
        private List<HiveObjectPrivilege> list_db_privileges(
                final String principalName,
                final PrincipalType principalType,
                String catName,
                final String dbName)
            ...
            try {
                ...
                if (principalName == null) {
                    ...
                    return getMS().listPrincipalDBGrants(
        ...
        private List<HiveObjectPrivilege> list_table_privileges(
            ...
            try {
                ...
                return getMS().listAllTableGrants(
                                principalName, principalType, catName, dbName, tableName);
            } catch (MetaException e) {
                throw e;
            } catch (Exception e) {
                throw new RuntimeException(e);
        ...
        public String get_delegation_token(
                String token_owner, String renewer_kerberos_principal_name) throws TException {
            startFunction("get_delegation_token");
            String ret = null;
            Exception ex = null;
            try {
                ret =
                        HiveMetaStore.getDelegationToken(
                                token_owner, renewer_kerberos_principal_name, getIPAddress());
        ...
        @Override
        public boolean add_token(String token_identifier, String delegation_token)
                throws TException {
        ...
        public void markPartitionForEvent(
                ...
                final PartitionEventType evtType)
                throws TException {
            ...
            try {
                ...
                if (transactionalListeners.size() > 0) {
                    LoadPartitionDoneEvent lpde =
                            new LoadPartitionDoneEvent(true, tbl, partName, this);
                    for (MetaStoreEventListener transactionalListener : transactionalListeners) {
        ...
        public boolean isPartitionMarkedForEvent(
            ...
            try {
                ret =
                        getMS().isPartitionMarkedForEvent(
                                        parsedDbName[CAT_NAME],
                                        parsedDbName[DB_NAME],
                                        tbl_name,
                                        partName,
        ...
        public boolean partition_name_has_valid_characters(
            ...
            try {
                if (throw_exception) {
                    ...
                    ret =
                            MetaStoreUtils.partitionNameHasValidCharacters(
                                    part_vals, partitionValidationPattern);
                }
            } catch (Exception e) {
                ex = e;
        ...
        private void validateFunctionInfo(Function func)
                throws InvalidObjectException, MetaException {
            if (!MetaStoreUtils.validateName(func.getFunctionName(), null)) {
                throw new InvalidObjectException(
                        func.getFunctionName() + " is not a valid object name");
            }
            ...
            try {
                ...
                if (!func.isSetOwnerName()) {
                    try {
                        ...
                        LOG.error(
                                "Cannot obtain username from the session to create a function", ex);
                ...
                if (db == null) {
                    throw new NoSuchObjectException(
                            "The database " + func.getDbName() + " does not exist");
                }

                Function existingFunc =
                        ms.getFunction(catName, func.getDbName(), func.getFunctionName());
        ...
        public void drop_function(String dbName, String funcName)
            ...
            try {
                ...
                Boolean isSourceOfReplication =
                        ReplChangeManager.isSourceOfReplication(
                                get_database_core(parsedDbName[CAT_NAME], parsedDbName[DB_NAME]));
                firePreEvent(new PreDropFunctionEvent(func, this));
                // if copy of jar to change management fails we fail the metastore transaction,
                // since the
                // user might delete the jars on HDFS externally after dropping the function, hence
                // having
                ...
                // if the operation on metastore fails, we don't do anything in change management,
                // but fail
                // the metastore transaction, as having a copy of the jar in change management is
                // not going
        ...
        public GetAllFunctionsResponse get_all_functions() throws MetaException {
            ...
            try {
                ...
                // configuration) because all the default UDFs are in that catalog, and I think
                // that's
        ...
        public OpenTxnsResponse open_txns(OpenTxnRequest rqst) throws TException {
            ...
            if (txnIds != null && listeners != null && !listeners.isEmpty()) {
                MetaStoreListenerNotifier.notifyEvent(
                        listeners, EventType.OPEN_TXN, new OpenTxnEvent(txnIds, this));
            }
            return response;
        }
        ...
        public void abort_txn(AbortTxnRequest rqst) throws TException {
            ...
            if (listeners != null && !listeners.isEmpty()) {
                MetaStoreListenerNotifier.notifyEvent(
                        listeners, EventType.ABORT_TXN, new AbortTxnEvent(rqst.getTxnid(), this));
            }
        }
        ...
        public void abort_txns(AbortTxnsRequest rqst) throws TException {
            ...
            if (listeners != null && !listeners.isEmpty()) {
                for (Long txnId : rqst.getTxn_ids()) {
                    MetaStoreListenerNotifier.notifyEvent(
                            listeners, EventType.ABORT_TXN, new AbortTxnEvent(txnId, this));
                }
            }
        }
        ...
        public void commit_txn(CommitTxnRequest rqst) throws TException {
            ...
            if (listeners != null && !listeners.isEmpty()) {
                MetaStoreListenerNotifier.notifyEvent(
                        listeners, EventType.COMMIT_TXN, new CommitTxnEvent(rqst.getTxnid(), this));
            }
        }
        ...
        public AllocateTableWriteIdsResponse allocate_table_write_ids(
            ...
            if (listeners != null && !listeners.isEmpty()) {
                MetaStoreListenerNotifier.notifyEvent(
                        listeners,
                        EventType.ALLOC_WRITE_ID,
                        new AllocWriteIdEvent(
                                response.getTxnToWriteIds(),
                                rqst.getDbName(),
                                rqst.getTableName(),
                                this));
            }
        ...
        public GetPrincipalsInRoleResponse get_principals_in_role(
            ...
            try {
                response =
                        new GetPrincipalsInRoleResponse(
                                getMS().listRoleMembers(request.getRoleName()));
            } catch (MetaException e) {
                throw e;
            } catch (Exception e) {
                ex = e;
        ...
        public boolean set_aggr_stats_for(SetPartitionsStatsRequest request) throws TException {
            ...
            String catName =
                    statsDesc.isSetCatName() ? statsDesc.getCatName() : getDefaultCatalog(conf);
            ...
            if (statsDesc.isIsTblLevel()) {
                ...
                for (Entry<String, ColumnStatistics> entry : newStatsMap.entrySet()) {
                    ...
                    if (request.isSetNeedMerge() && request.isNeedMerge()) {
                        ...
                        MetaStoreUtils.getMergableCols(
                                csNew, mapToPart.get(entry.getKey()).getParameters());
                        // we merge those that can be merged
                        if (csOld != null
                                && csOld.getStatsObjSize() != 0
        ...
        public NotificationEventResponse get_next_notification(NotificationEventRequest rqst)
            ...
            try {
                ...
                LOG.error(
                        "Not authorized to make the get_next_notification call. You can try to disable "
        ...
        public CurrentNotificationEventId get_current_notificationEventId() throws TException {
            try {
                ...
                LOG.error(
                        "Not authorized to make the get_current_notificationEventId call. You can try to disable "
                                + ConfVars.EVENT_DB_NOTIFICATION_API_AUTH.toString(),
                        ex);
                throw new TException(ex);
            }
        ...
        public NotificationEventsCountResponse get_notification_events_count(
            ...
            try {
                ...
                LOG.error(
                        "Not authorized to make the get_notification_events_count call. You can try to disable "
                                + ConfVars.EVENT_DB_NOTIFICATION_API_AUTH.toString(),
                        ex);
                throw new TException(ex);
            }
        ...
        private void authorizeProxyPrivilege() throws Exception {
            ...
            if (!MetaStoreUtils.checkUserHasHostProxyPrivileges(user, conf, getIPAddress())) {
                throw new MetaException(
                        "User " + user + " is not allowed to perform this API call");
            }
        }
        ...
        public FireEventResponse fire_listener_event(FireEventRequest rqst) throws TException {
            switch (rqst.getData().getSetField()) {
                case INSERT_DATA:
                     ...
                     */
                    MetaStoreListenerNotifier.notifyEvent(
                            transactionalListeners, EventType.INSERT, event);
        ...
        public GetFileMetadataByExprResult get_file_metadata_by_expr(
                GetFileMetadataByExprRequest req) throws TException {
            ...
            FileMetadataExprType type =
                    req.isSetType() ? req.getType() : FileMetadataExprType.ORC_SARG;
            ...
            getMS().getFileMetadataByExpr(
                            fileIds, type, req.getExpr(), metadatas, ppdResults, eliminated);
            for (int i = 0; i < fileIds.size(); ++i) {
                ...
                ByteBuffer ppdResult =
                        eliminated[i] ? null : handleReadOnlyBufferForThrift(ppdResults[i]);
                mpr.setIncludeBitset(ppdResult);
                if (needMetadata) {
                    ByteBuffer metadata =
                            eliminated[i] ? null : handleReadOnlyBufferForThrift(metadatas[i]);
        ...
        private static final Map<Long, ByteBuffer> EMPTY_MAP_FM1 = new HashMap<>(1);
        private static final Map<Long, MetadataPpdResult> EMPTY_MAP_FM2 = new HashMap<>(1);
        ...
        public CacheFileMetadataResult cache_file_metadata(CacheFileMetadataRequest req)
            ...
            try {
                ...
                if (!isPartitioned) {
                    ...
                    if (type == null) {
                        throw new MetaException(
                                "The operation is not supported for " + tableInputFormat);
                    }
                    ...
                    if (partName != null) {
                        ...
                        partNames =
                                ms.listPartitionNames(
                                        DEFAULT_CATALOG_NAME, dbName, tblName, (short) -1);
                    } else {
                        throw new MetaException("Table is partitioned");
                    }
                    int batchSize =
                            MetastoreConf.getIntVar(conf, ConfVars.BATCH_RETRIEVE_OBJECTS_MAX);
                    int index = 0;
                    ...
                    while (index < partNames.size()) {
                        ...
                        List<Partition> parts =
                                ms.getPartitionsByNames(
                                        DEFAULT_CATALOG_NAME, dbName, tblName, nameBatch);
                        for (Partition part : parts) {
                            if (!part.isSetSd() || !part.getSd().isSetLocation()) {
                                throw new MetaException(
                                        "Partition does not have storage location;"
                                                + " this operation is not supported on views");
                            }
                            String inputFormat =
                                    part.getSd().isSetInputFormat()
                                            ? part.getSd().getInputFormat()
                                            : tableInputFormat;
                            FileMetadataExprType type =
                                    expressionProxy.getMetadataType(inputFormat);
                            if (type == null) {
                                ++failCount;
                                if (failFormats == null) {
                                ...
                                ++successCount;
                                fileMetadataManager.queueCacheMetadata(
                                        part.getSd().getLocation(), type);
                            }
                        }
                    }
                    ...
                    if (failCount > 0) {
                        String errorMsg =
                                "The operation failed for "
                                        ...
                                        + "succeeded for "
                                        + successCount
                                        + " partitions; unsupported formats: ";
                        boolean isFirst = true;
        ...
        public NotNullConstraintsResponse get_not_null_constraints(
                NotNullConstraintsRequest request) throws TException {
            String catName =
                    request.isSetCatName() ? request.getCatName() : getDefaultCatalog(conf);
            String db_name = request.getDb_name();
            String tbl_name = request.getTbl_name();
        ...
        public WMCreateResourcePlanResponse create_resource_plan(
            ...
            int defaultPoolSize =
                    MetastoreConf.getIntVar(conf, MetastoreConf.ConfVars.WM_DEFAULT_POOL_SIZE);
            WMResourcePlan plan = request.getResourcePlan();
            if (defaultPoolSize > 0 && plan.isSetQueryParallelism()) {
                // If the default pool is not disabled, override the size with the specified
                // parallelism.
        ...
        public WMGetAllResourcePlanResponse get_all_resource_plans(
                WMGetAllResourcePlanRequest request) throws MetaException, TException {
            try {
        ...
        public WMAlterResourcePlanResponse alter_resource_plan(WMAlterResourcePlanRequest request)
            ...
            try {
                if (((request.isIsEnableAndActivate() ? 1 : 0)
                                + (request.isIsReplace() ? 1 : 0)
                                + (request.isIsForceDeactivate() ? 1 : 0))
                        > 1) {
                ...
                WMFullResourcePlan fullPlanAfterAlter =
                        getMS().alterResourcePlan(
        ...
        public WMGetTriggersForResourePlanResponse get_triggers_for_resourceplan(
            ...
            try {
                ...
                WMGetTriggersForResourePlanResponse response =
                ...
                try {
                    ...
                    if (!transactionalListeners.isEmpty()) {
                        transactionalListenersResponses =
                                MetaStoreListenerNotifier.notifyEvent(
                                        ...
                                        new CreateISchemaEvent(true, this, schema));
                    }
                    success = ms.commitTransaction();
                } finally {
                    if (!success) {
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                listeners,
                                EventType.CREATE_ISCHEMA,
                                new CreateISchemaEvent(success, this, schema),
                                null,
                                transactionalListenersResponses,
                                ms);
                    }
                ...
                try {
                    ...
                    if (!transactionalListeners.isEmpty()) {
                        transactionalListenersResponses =
                                MetaStoreListenerNotifier.notifyEvent(
                                        ...
                                        new AlterISchemaEvent(
                                                true, this, oldSchema, rqst.getNewSchema()));
                    }
                    success = ms.commitTransaction();
                } finally {
                    if (!success) {
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                ...
                                EventType.ALTER_ISCHEMA,
                                new AlterISchemaEvent(
                                        success, this, oldSchema, rqst.getNewSchema()),
                                null,
                                transactionalListenersResponses,
                                ms);
                    }
        ...
        public void drop_ischema(ISchemaName schemaName) throws TException {
            ...
            RawStore ms = getMS();
            try {
                // look for any valid versions.  This will also throw NoSuchObjectException if the
                // schema
                ...
                try {
                    ...
                    if (!transactionalListeners.isEmpty()) {
                        transactionalListenersResponses =
                                MetaStoreListenerNotifier.notifyEvent(
                                        ...
                                        new DropISchemaEvent(true, this, schema));
                    }
                    success = ms.commitTransaction();
                } finally {
                    if (!success) {
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                listeners,
                                EventType.DROP_ISCHEMA,
                                new DropISchemaEvent(success, this, schema),
                                null,
                                transactionalListenersResponses,
                                ms);
                    }
                ...
                try {
                    ...
                    if (!transactionalListeners.isEmpty()) {
                        transactionalListenersResponses =
                                MetaStoreListenerNotifier.notifyEvent(
                                        ...
                                        new AddSchemaVersionEvent(true, this, schemaVersion));
                    }
                    success = ms.commitTransaction();
                } finally {
                    if (!success) {
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                listeners,
                                EventType.ADD_SCHEMA_VERSION,
                                new AddSchemaVersionEvent(success, this, schemaVersion),
                                null,
                                transactionalListenersResponses,
                                ms);
                    }
        ...
        public SchemaVersion get_schema_version(SchemaVersionDescriptor version) throws TException {
            ...
            try {
                ...
                firePreEvent(
                        new PreReadhSchemaVersionEvent(
                                this, Collections.singletonList(schemaVersion)));
        ...
        public SchemaVersion get_schema_latest_version(ISchemaName schemaName) throws TException {
            ...
            try {
                ...
                if (schemaVersion == null) {
                    throw new NoSuchObjectException(
                            "No versions of schema " + schemaName + "exist");
                }
                firePreEvent(
                        new PreReadhSchemaVersionEvent(
                                this, Collections.singletonList(schemaVersion)));
                ...
                try {
                    ...
                    if (!transactionalListeners.isEmpty()) {
                        transactionalListenersResponses =
                                MetaStoreListenerNotifier.notifyEvent(
                                        ...
                                        new DropSchemaVersionEvent(true, this, schemaVersion));
                    }
                    success = ms.commitTransaction();
                } finally {
                    if (!success) {
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                listeners,
                                EventType.DROP_SCHEMA_VERSION,
                                new DropSchemaVersionEvent(success, this, schemaVersion),
                                null,
                                transactionalListenersResponses,
                                ms);
                    }
        ...
        public FindSchemasByColsResp get_schemas_by_cols(FindSchemasByColsRqst rqst)
            ...
            try {
                schemaVersions =
                        getMS().getSchemaVersionsByColumns(
                                        rqst.getColName(), rqst.getColNamespace(), rqst.getType());
                firePreEvent(new PreReadhSchemaVersionEvent(this, schemaVersions));
                final List<SchemaVersionDescriptor> entries =
                        new ArrayList<>(schemaVersions.size());
        ...
        public void map_schema_version_to_serde(MapSchemaVersionToSerdeRequest rqst)
            ...
            try {
                ...
                firePreEvent(
                        new PreAlterSchemaVersionEvent(this, oldSchemaVersion, newSchemaVersion));
                Map<String, String> transactionalListenersResponses = Collections.emptyMap();
                ms.openTransaction();
                try {
                    ...
                    if (!transactionalListeners.isEmpty()) {
                        transactionalListenersResponses =
                                MetaStoreListenerNotifier.notifyEvent(
                                        ...
                                        new AlterSchemaVersionEvent(
                                                true, this, oldSchemaVersion, newSchemaVersion));
                    }
                    success = ms.commitTransaction();
                } finally {
                    if (!success) {
                    ...
                    if (!listeners.isEmpty()) {
                        MetaStoreListenerNotifier.notifyEvent(
                                ...
                                EventType.ALTER_SCHEMA_VERSION,
                                new AlterSchemaVersionEvent(
                                        success, this, oldSchemaVersion, newSchemaVersion),
                                null,
                                transactionalListenersResponses,
                                ms);
                    }
        ...
        public SerDeInfo get_serde(GetSerdeRequest rqst) throws TException {
            ...
            try {
                ...
                if (serde == null) {
                    throw new NoSuchObjectException(
                            "No serde named " + rqst.getSerdeName() + " exists");
                }
                return serde;
            } catch (MetaException e) {
        ...
        @Override
        public LockResponse get_lock_materialization_rebuild(
                String dbName, String tableName, long txnId) throws TException {
            return getTxnHandler().lockMaterializationRebuild(dbName, tableName, txnId);
        }

        @Override
        public boolean heartbeat_lock_materialization_rebuild(
                String dbName, String tableName, long txnId) throws TException {
        ...
        public List<RuntimeStat> get_runtime_stats(GetRuntimeStatsRequest rqst) throws TException {
            ...
            try {
                List<RuntimeStat> res =
                        getMS().getRuntimeStats(rqst.getMaxWeight(), rqst.getMaxCreateTime());
                return res;
            } catch (MetaException e) {
                LOG.error("Caught exception", e);
                ex = e;
    ...
    private static IHMSHandler newRetryingHMSHandler(
            IHMSHandler baseHandler, Configuration conf, boolean local) throws MetaException {
        ...
        HiveMetastoreCli(Configuration configuration) {
            ...
            OPTIONS.addOption(
                    OptionBuilder.hasArg()
                            .withArgName("port")
                            .withDescription("Hive Metastore port number, default:" + this.port)
                            .create('p'));
        }
     ...
     * @throws Throwable
     */
    public static void startMetaStore(int port, HadoopThriftAuthBridge bridge, Configuration conf)
            throws Throwable {
    ...
    public static void startMetaStore(
        ...
        try {
            ...
            if (useSasl) {
                ...
                if (useFramedTransport) {
                    throw new HiveMetaException(
                            "Framed transport is not supported with SASL enabled.");
                }
                saslServer =
                        bridge.createServer(
                ...
                transFactory =
                        saslServer.createTransportFactory(
                                MetaStoreUtils.getMetaStoreSaslProperties(conf, useSSL));
                processor =
                        saslServer.wrapNonAssumingProcessor(
            ...
            if (!useSSL) {
                ...
                if (keyStorePath.isEmpty()) {
                    throw new IllegalArgumentException(
                            ConfVars.SSL_KEYSTORE_PATH.toString()
                                    + " Not configured for SSL connection");
                }
                String keyStorePassword =
                        MetastoreConf.getPassword(
                                conf, MetastoreConf.ConfVars.SSL_KEYSTORE_PASSWORD);

                // enable SSL support for HMS
                List<String> sslVersionBlacklist = new ArrayList<>();
                for (String sslVersion :
                        MetastoreConf.getVar(conf, ConfVars.SSL_PROTOCOL_BLACKLIST).split(",")) {
                    sslVersionBlacklist.add(sslVersion);
                }

                serverSocket =
                        SecurityUtils.getServerSSLSocket(
            ...
            TServerEventHandler tServerEventHandler =
                    new TServerEventHandler() {
                        ...
                        public ServerContext createContext(
                                TProtocol tProtocol, TProtocol tProtocol1) {
    ...
    private static void startMetaStoreThreads(
        ...
        // until after the TServer has started, but once TServer.serve is called we aren't given
        // back
        // control.
        Thread t =
                new Thread() {
                    @Override
                    public void run() {
                        // This is a massive hack.  The compactor threads have to access packages in
                        // ql (such as
                        // AcidInputFormat).  ql depends on metastore so we can't directly access
                        // those.  To deal
                        // with this the compactor thread classes have been put in ql and they are
                        // instantiated here
                        // dyanmically.  This is not ideal but it avoids a massive refactoring of
                        // Hive packages.
                        //
                        // Wrap the start of the threads in a catch Throwable loop so that any
                        // failures
                        ...
                        try {
                            ...
                            LOG.warn(
                                    "Could not initiate the JvmPauseMonitor thread."
                                            + " GCs and Pauses may not be "
                                            + "warned upon.",
                                    t);
                        }

                        try {
                            // Per the javadocs on Condition, do not depend on the condition alone
                            // as a start gate
    ...
    protected static void startStatsUpdater(Configuration conf) throws Exception {
        StatsUpdateMode mode =
                StatsUpdateMode.valueOf(
     ...
     */
    private static void startupShutdownMessage(
            Class<?> clazz, String[] args, final org.slf4j.Logger LOG) {

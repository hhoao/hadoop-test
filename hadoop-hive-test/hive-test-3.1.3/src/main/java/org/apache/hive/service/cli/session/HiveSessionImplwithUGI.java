...
public class HiveSessionImplwithUGI extends HiveSessionImpl {
    ...
    public void setSessionUGI(String owner) throws HiveSQLException {
        ...
        try {
            sessionUgi = UserGroupInformation.getCurrentUser();
        } catch (IOException e) {
            ...
            try {
                ...
                try {
                    ...
                    throw new HiveSQLException(
                            "Could not clean up file-system handles for UGI: " + sessionUgi, ioe);
                }
            }
        }
    }
    ...
    /**
     * Enable delegation token for the session save the token string and set the token.signature in
     * hive conf. The metastore client uses this token.signature to determine where to use kerberos
     * or delegation token
     *
    ...
    private IMetaStoreClient getMetaStoreClient(boolean retryInCaseOfTokenExpiration)
            throws HiveSQLException {
        try {
                ...
                try {
                    setDelegationToken(
                            Hive.get(getHiveConf())
                                    .getDelegationToken(sessionUgi.getUserName(), getUserName()));
                } catch (HiveException e2) {
                    throw new HiveSQLException(
                            "Error connect metastore to setup impersonation: " + e2, e2);
                }

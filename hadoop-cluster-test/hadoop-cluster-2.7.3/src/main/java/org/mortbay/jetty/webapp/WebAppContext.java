...
/* ------------------------------------------------------------ */
/**
 * Web Application Context Handler. The WebAppContext handler is an extension of ContextHandler that
 * coordinates the construction and configuration of nested handlers: {@link SecurityHandler},
 * {@link SessionHandler} and {@link ServletHandler}. The handlers are configured by pluggable
 * configuration classes, with the default being {@link WebXmlConfiguration} and {@link
 * JettyWebXmlConfiguration}.
 *
 * @org.apache.xbean.XBean description="Creates a servlet web application at a given context from a
 *     resource base"
 * @author gregw
 */
public class WebAppContext extends Context {
    ...
    private boolean _parentLoaderPriority =
            Boolean.getBoolean("org.mortbay.jetty.webapp.parentLoaderPriority");
    ...
    public static ContextHandler getCurrentWebAppContext() {
        ...
        if (context != null) {
            ...
            if (handler instanceof WebAppContext) return (ContextHandler) handler;
        }
        return null;
    }
    ...
    /* ------------------------------------------------------------ */
    /**
     * Add Web Applications. Add auto webapplications to the server. The name of the webapp
     * directory or war is used as the context name. If the webapp matches the rootWebApp it is
     * added as the "/" context.
     *
     * @param server Must not be <code>null</code>
     * @param webapps Directory file name or URL to look for auto webapplication.
     * @param defaults The defaults xml filename or URL which is loaded before any in the web app.
     *     Must respect the web.dtd. If null the default defaults file is used. If the empty string,
     *     then no defaults file is used.
     ...
     */
    public static void addWebApplications(
            Server server,
            String webapps,
            String defaults,
            boolean extract,
    ...
    public static void addWebApplications(
        ...
        HandlerCollection contexts =
                (HandlerCollection) server.getChildHandlerByClass(ContextHandlerCollection.class);
        if (contexts == null)
            contexts = (HandlerCollection) server.getChildHandlerByClass(HandlerCollection.class);

        addWebApplications(
                contexts, webapps, defaults, configurations, extract, java2CompliantClassLoader);
    }

    /* ------------------------------------------------------------ */
    /**
     * Add Web Applications. Add auto webapplications to the server. The name of the webapp
     * directory or war is used as the context name. If the webapp is called "root" it is added as
     * the "/" context.
     *
    ...
    /* ------------------------------------------------------------ */
    /**
     * Set Resource Alias. Resource aliases map resource uri's within a context. They may optionally
     * be used by a handler when looking for a resource.
     *
     * @param alias
     * @param uri
     */
    public void setResourceAlias(String alias, String uri) {
        if (_resourceAliases == null) _resourceAliases = new HashMap(5);
        _resourceAliases.put(alias, uri);
    }

    /* ------------------------------------------------------------ */
    public Map getResourceAliases() {
        if (_resourceAliases == null) return null;
    ...
    public void setClassLoader(ClassLoader classLoader) {
        ...
        //        {
        //            Log.info("NOTE: detected a classloader which is not an instance of
        // WebAppClassLoader being set on WebAppContext, some typical class and resource locations
        // may be missing on: " + toString() );
        //        }
    ...
    public Resource getResource(String uriInContext) throws MalformedURLException {
        ...
        if (ioe != null && ioe instanceof MalformedURLException) throw (MalformedURLException) ioe;

        return resource;
    }

    /* ------------------------------------------------------------ */
    /** @see ContextHandler#handle(String, HttpServletRequest, HttpServletResponse, int) */
    public void handle(
        ...
        } else super.handle(target, request, response, dispatch);
    }
    ...
    public void dumpUrl() {
        ...
        for (int i = 0; i < connectors.length; i++) {
            ...
            if (displayName == null) displayName = "WebApp@" + connectors.hashCode();
    ...
    protected void doStop() throws Exception {
        ...
        try {
            ...
            if (_tmpDir != null && !_isExistingTmpDir && !isTempWorkDirectory()) // _tmpDir!=null &&
            // !"work".equals(_tmpDir.getName()))
            {
                IO.delete(_tmpDir);
                _tmpDir = null;
            }
    ...
    /* ------------------------------------------------------------ */
    /**
     * The default descriptor is a web.xml format file that is applied to the context before the
    ...
    /* ------------------------------------------------------------ */
    /**
     * The override descriptor is a web.xml format file that is applied to the context after the
     * standard WEB-INF/web.xml
     *
     ...
     *   <li>Else create dir in $(java.io.tmpdir). Set delete on exit. Delete contents if dir
     *       already exists.
     * </ol>
     *
     * @return
    ...
    /**
     * Check if the _tmpDir itself is called "work", or if the _tmpDir is in a directory called
     * "work".
     *
     * @return
     */
    public boolean isTempWorkDirectory() {
        if (_tmpDir == null) return false;
        if (_tmpDir.getName().equalsIgnoreCase("work")) return true;
    ...
    public String getWar() {
        if (_war == null) _war = getResourceBase();
        return _war;
    }
    ...
    protected void loadConfigurations() throws Exception {
        ...
        for (int i = 0; i < _configurations.length; i++) {
            _configurations[i] =
                    (Configuration)
                            Loader.loadClass(this.getClass(), _configurationClasses[i])
                                    .newInstance();
        }
    }
    ...
    /* ------------------------------------------------------------ */
    /**
     * Resolve Web App directory If the BaseResource has not been set, use the war resource to
    ...
    protected void resolveWebApp() throws IOException {
        ...
        if (web_app == null) {
            ...
            if (web_app.exists()
                    && !web_app.isDirectory()
                    && !web_app.toString().startsWith("jar:")) {
            ...
            if (web_app.exists()
                ...
                if (web_app.getFile() != null && web_app.getFile().isDirectory()) {
                    ...
                    String urlString = web_app.getURL().toExternalForm().trim();
                    int endOfJarUrl = urlString.indexOf("!/");
                    String subEntryName =
                            (endOfJarUrl + 2 < urlString.length()
                                    ? urlString.substring(endOfJarUrl + 2)
                                    : null);
                    if (subEntryName != null) {
                        extractedWebAppDir = new File(extractedWebAppDir, subEntryName);
                    }
                }
            ...
            if (!web_app.exists() || !web_app.isDirectory()) {
                ...
                throw new FileNotFoundException(_war);
            }
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param configurations The configuration class names. If setConfigurations is not called these
     *     classes are used to create a configurations array.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param descriptor the web.xml descriptor to use. If set to null, WEB-INF/web.xml is used if
     *     it exists.
     */
    ...
    public void setEventListeners(EventListener[] eventListeners) {
        if (_sessionHandler != null) _sessionHandler.clearEventListeners();
        ...
        for (int i = 0; eventListeners != null && i < eventListeners.length; i++) {
            ...
            if ((listener instanceof HttpSessionActivationListener)
                ...
                if (_sessionHandler != null) _sessionHandler.addEventListener(listener);
            }
        }
    }
    ...
    public void addEventListener(EventListener listener) {
        setEventListeners(
                (EventListener[])
                        LazyList.addToArray(getEventListeners(), listener, EventListener.class));
    }
    ...
    public void setTempDirectory(File dir) {
        ...
        if (dir != null && !dir.exists()) {
            dir.mkdir();
            dir.deleteOnExit();
        } else if (dir != null) _isExistingTmpDir = true;
    ...
    /* ------------------------------------------------------------ */
    /**
     * @return Comma or semicolon separated path of filenames or URLs pointing to directories or jar
     *     files. Directories should end with '/'.
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @param extraClasspath Comma or semicolon separated path of filenames or URLs pointing to
    ...
    protected void startContext() throws Exception {
        // Configure defaults
        for (int i = 0; i < _configurations.length; i++) _configurations[i].configureDefaults();
        ...
        // Configure webapp
        for (int i = 0; i < _configurations.length; i++) _configurations[i].configureWebApp();

        super.startContext();
    }

    /**
     * Create a canonical name for a webapp tmp directory. The form of the name is:
     * "Jetty_"+host+"_"+port+"__"+resourceBase+"_"+context+"_"+virtualhost+base36 hashcode of whole
     * string
     *
     * <p>host and port uniquely identify the server context and virtual host uniquely identify the
     * webapp
     *
     * @return
     */

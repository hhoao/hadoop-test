...
public class GuiceFilter implements Filter {
    ...
    static volatile InheritableThreadLocal<FilterPipeline> pipeline =
            new InheritableThreadLocal<>();

    /** We allow both the static and dynamic versions of the pipeline to exist. */
    @Inject private final FilterPipeline injectedPipeline = null;
    ...
    static void setPipeline(FilterPipeline pipeline) {
        ...
        if (GuiceFilter.pipeline.get() != null
                && GuiceFilter.pipeline.get() instanceof ManagedFilterPipeline) {
            Logger.getLogger(GuiceFilter.class.getName()).warning(MULTIPLE_INJECTORS_WARNING);
        }

        // We overwrite the default pipeline
        GuiceFilter.pipeline.set(pipeline);
    }

    // VisibleForTesting
    static void reset() {
        pipeline.set(new DefaultFilterPipeline());
    }
    ...
    public void doFilter(
        ...
        FilterPipeline filterPipeline =
                null != injectedPipeline ? injectedPipeline : pipeline.get();
